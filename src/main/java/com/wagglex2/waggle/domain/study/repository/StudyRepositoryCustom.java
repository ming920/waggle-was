package com.wagglex2.waggle.domain.study.repository;

import com.wagglex2.waggle.domain.study.dto.request.StudySearchCondition;
import com.wagglex2.waggle.domain.study.dto.response.StudySummaryResponseDto;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;

import java.util.List;

/**
 * QueryDSL을 활용한 Study 커스텀 리포지토리
 */
public interface StudyRepositoryCustom {
    /**
     * 검색 조건({@link StudySearchCondition})에 맞는 스터디 요약 DTO를 페이징 조회
     * @param condition 검색 조건 ({@link StudySearchCondition})
     * @param pageable  페이징 및 정렬 정보 ({@link org.springframework.data.domain.Pageable})
     * @return 조건에 맞는 스터디 요약 DTO 페이지 ({@link org.springframework.data.domain.Page}&lt;{@link StudySummaryResponseDto}&gt;)
     */
    Page<StudySummaryResponseDto> getStudySummaries(Long viewerId, StudySearchCondition condition, Pageable pageable);

    /**
     * 주어진 Study ID 목록에 해당하는 스터디 요약 정보를 조회한다.
     *
     * @param studyIds 조회할 Study ID 목록
     * @return 입력 순서에 맞춘 {@code List<StudySummaryResponseDto>}
     */
    List<StudySummaryResponseDto> getStudySummariesByIds(List<Long> studyIds);
}
